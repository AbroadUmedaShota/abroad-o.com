...
const pages = walk(output).filter((file) => file.endsWith('.html') && path.relative(output, file).replaceAll('\\', '/') !== 'slick/largeformat.html');
if (pages.length !== 47) throw new Error(`Expected 47 generated pages, found ${pages.length}.`);
