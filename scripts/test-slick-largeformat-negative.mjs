...
test('rejects return-link style and geometry mutations', () => {
  const expected = { overflow: 0, header: { height: 100 }, bodyHeight: 200, largestContainer: 100, returnLink: { color: 'rgb(204, 204, 204)' }, cta: { display: 'inline-block', fontSize: '13px' } };
  assert.throws(() => assertSlickGeometry({ overflow: 0, header: 100, body: 200, container: 100, returnLink: { color: 'rgb(85, 85, 85)' }, cta: { display: 'inline-block', fontSize: '13px' } }, expected));
  assert.throws(() => assertSlickGeometry({ overflow: 1, header: 100, body: 200, container: 100, returnLink: { color: 'rgb(204, 204, 204)' }, cta: { display: 'inline-block', fontSize: '13px' } }, expected));
});
