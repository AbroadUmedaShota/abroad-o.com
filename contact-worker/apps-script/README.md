...
自動返信が有効な場合、送信時に同じ台帳の名前付き範囲`CONTACT_AUTOREPLY_PUBLISHED`から、`版、件名、本文、公開日時、公開者`の1行を読み取ります。公開テンプレートは送信のたびに検証し、削除・破損・文字数超過・不正な差し込み項目があれば、コード内の標準文面へ自動的にフォールバックします。フォールバック時のログには理由コードだけを残し、テンプレート本文、問い合わせ内容、メールアドレスは記録しません。

台帳での編集・プレビュー・公開・ロールバックは[`template-editor`](../template-editor/README.md)のコンテナバインド型Apps Scriptが担当します。下書きの変更は公開操作まで実メールへ影響せず、公開後は受信Webアプリの再デプロイなしで次回の自動返信から反映されます。社内通知、宛先、差出人、送信ON/OFFは編集対象ではありません。
